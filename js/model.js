...
const getPosition = function () {
  if (navigator.geolocation) {
    ...
    errDisplay.style.display = "block";
    errDisplay.textContent = "your browser doesn't support getting your location, search instead.";
  }
};
...
export const loadCityInfo = async function (inputCity, error) {
  try {
    const res = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=4e95fd070598460ebb2110205221306&q=${inputCity}&days=1&aqi=yes&alerts=yes`);

    if (!res.ok) throw new Error(error);
    let dataCityInfo = await res.json();
    ///fetching country isi code
    const resCountryIso = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${dataCityInfo.location.lat}&lon=${dataCityInfo.location.lon}&appid=3262716e26ad022afb3c61d6f5e11348`);
    if (!resCountryIso.ok) throw new Error("unable to fetch country code, check your internet connection🧨");
    ...
    const resCountry = await fetch(`https://restcountries.com/v2/alpha/${isiCode}`);
    if (!resCountry.ok) throw new Error("unable to fecth country information, check your internet connection🧨");
    const dataCountry = await resCountry.json();
    let { currencies, flag, languages, name, population } = dataCountry;
    let allCountryData = [currencies, flag, languages, name, population, accWeather];
    ...
    if (cityState.bookmarks.some((b) => b.location.lat === dataCityInfo.location.lat && b.location.lon === dataCityInfo.location.lon)) {
...
export const addBookmark = function (city) {
  ...
  if (city.location.name === cityState.data.location.name) cityState.data.bookmarked = true;
  persistBookmark();
};

export const deleteBookmark = function (cityName) {
  const index = cityState.bookmarks.findIndex((el) => el.location.name === cityName.location.name);
...
export const init = function () {
  ...
  cityState.bookmarks.forEach(async (a) => {
    const res = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=4e95fd070598460ebb2110205221306&q=${a.location.name}&days=1&aqi=yes&alerts=yes`);
    ...
    <img
    class ="weather-icon-deg" src="${dataUpdate.current.condition.icon}" alt=""/>
    </div>
    <div class="book-weather-status">
      <p class="book-humidity"><i class="ph-drop-half-bottom"></i>${dataUpdate.current.humidity}%</p>
      <p class="book-wind-speed">
        <i class="weather-icons ph-wind"></i>${dataUpdate.current.wind_kph}Km/h
      </p>
      <p class="book-rain-chance">
        <i class="weather-icons ph-cloud-rain"></i>${dataUpdate.forecast.forecastday[0].day.daily_chance_of_rain}%
      </p>
    </div>
  </div>`;
    document.querySelector(".bookmark-container").insertAdjacentHTML("beforeend", html);
  });

  cityState.bookmarks = cityState.bookmarksUpdate;
  if (cityState.bookmarks.length === 0) document.querySelector(".bookmark-error").innerHTML = "No bookmark yet!";
};
init();
